    ...
    <main className="min-h-screen bg-background text-foreground">
      ...
      <section id="certificado" className="py-20 md:py-28 bg-cream border-b border-ink/10">
        <div className="max-w-3xl mx-auto px-6">
          ...
          <h2 className="mt-3 text-4xl md:text-6xl text-ink">Solicitar certificado de participação.</h2>
          <p className="mt-4 text-lg text-ink/70">Informe o nome que deve constar no certificado.</p>

          <form onSubmit={handleCertificado} className="mt-10 space-y-5">
            <div>
              <label htmlFor="nomeCert" className="block text-xs uppercase tracking-widest text-ink/60 mb-2">
            ...
            <button
              ...
              className="w-full md:w-auto px-10 py-4 text-base font-bold rounded-sm hover:opacity-90 transition disabled:opacity-50 bg-ink text-cream"
            >
              {certStatus === "sending" ? "ENVIANDO…" : "SOLICITAR CERTIFICADO →"}
            </button>

            {certStatus === "sent" && (
              <p className="text-sm text-emerald-600">Solicitação recebida! Em breve você receberá o certificado por e-mail.</p>
            )}
            {certStatus === "error" && (
              <p className="text-sm text-red-600">Não foi possível enviar agora. Tente novamente ou escreva para {INSCRICAO_EMAIL}.</p>
            )}
          </form>
        </div>
      </section>
